...
## create residuals
formula_mcsa = as.formula(paste0("expression ~ PAXgene_Age + Sex + RIN + PAXgene_flowcell + PAXgene_Batch + ", ct))
mcsa_res <- cqn_ls %>%
...
saveRDS(mcsa_res,paste0(outdir, "/MCSA/mcsa_res_", ct, ".rds"))
...
for (i in 1:102){
  ...
  formula_adni1 = as.formula(paste0("GenExpr ~ -1 + Gene + AGE_EXAM + PTGENDER + RIN + (1|Affy.Plate) + (1|SITE) + ", ct))
  fit.lmer <- lmer(formula_adni1, data = myData) 
  ## extract random effect
  ranef.fit.lmer <- ranef(fit.lmer)
  
  saveRDS(ranef.fit.lmer, file = paste0(outdir, "/ADNI/resids_batch_s91_", ct, "/itr_", itr, ".rds"))
}

for(i in 1:102){
  ranef.fit.lmer <- readRDS(paste0(outdir, "/ADNI/resids_batch_s91_", ct, "/itr_", i, ".rds"))
  if(i==1){
...
## calculate residuals
formula_adni2 = as.formula(paste0("GenExpr ~ AGE_EXAM + PTGENDER + RIN + AffyPlate.meanREE + SITE.meanREE + ", ct))
...
saveRDS(adni_res, file = paste0(outdir, "/ADNI/adni_res_", ct, ".rds"))